import React from 'react';
import { useModeAnimation } from './useModeAnimation';

const isMobile = typeof window !== 'undefined' && /Mobi|Android/i.test(navigator.userAgent);

const SwitchDarkMode = ({ isDarkMode, onDarkModeChange, ...props }) => {
  const { ref, toggleSwitchTheme, isDarkMode: internalDarkMode } = useModeAnimation({
    ...props,
    isDarkMode,
    onDarkModeChange,
  });

  return (
    <button
      ref={ref}
      onClick={toggleSwitchTheme}
      style={{
        background: 'none',
        border: 'none',
        padding: 0,
        cursor: 'pointer',
        outline: 'none',
      }}
    >
      {internalDarkMode ? <MoonIcon /> : <SunIcon />}
    </button>
  );
};

const MoonIcon = () => {
  const color = isMobile ? '#000000' : '#ffffff';
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill={color} xmlns="http://www.w3.org/2000/svg">
      <path
        d="M2.03 12.42C2.39 17.57 6.76 21.76 11.99 21.99C15.68 22.15 18.98 20.43 20.96 17.72C21.78 16.61 21.34 15.87 19.97 16.12C19.3 16.24 18.61 16.29 17.89 16.26C13 16.06 9 11.97 8.98 7.14C8.97 5.84 9.24 4.61 9.73 3.49C10.27 2.25 9.62 1.66 8.37 2.19C4.41 3.86 1.7 7.85 2.03 12.42Z"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
};

const SunIcon = () => {
  const color = isMobile ? ' #ffffff' : '#000000';
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill={color} xmlns="http://www.w3.org/2000/svg">
      <path
        d="M12 18.5C15.5899 18.5 18.5 15.5899 18.5 12C18.5 8.41015 15.5899 5.5 12 5.5C8.41015 5.5 5.5 8.41015 5.5 12C5.5 15.5899 8.41015 18.5 12 18.5Z"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M19.14 19.14L19.01 19.01M19.01 4.99L19.14 4.86L19.01 4.99ZM4.86 19.14L4.99 19.01L4.86 19.14ZM12 2.08V2V2.08ZM12 22V21.92V22ZM2.08 12H2H2.08ZM22 12H21.92H22ZM4.99 4.99L4.86 4.86L4.99 4.99Z"
        stroke={color}
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
};

export default SwitchDarkMode;
